...
const SupportChat: React.FC<SupportChatProps> = ({ primaryColor }) => {
  ...
  return (
    <div data-tour="ai-chat" className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-80 h-96 bg-white dark:bg-[#1d1d1f] rounded-3xl shadow-2xl border border-gray-100 dark:border-white/10 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-xs font-medium leading-relaxed ${
                  m.role === 'user' ? 'bg-gray-100 dark:bg-[#242427] text-gray-900 dark:text-gray-100' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                }`}>
                  {m.text}
                </div>
              </div>
            ))}
            {isTyping && <div className="text-[10px] text-gray-400 dark:text-gray-500 font-bold animate-pulse">Asistent piše...</div>}
          </div>
          <div className="p-3 border-t border-gray-50 dark:border-white/10 flex gap-2">
            <input 
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && handleSend()}
              placeholder="Postavite pitanje..."
              className="flex-1 bg-gray-50 dark:bg-[#242427] rounded-xl px-4 py-2 text-xs outline-none border border-transparent dark:text-gray-100 focus:border-blue-200"
            />
